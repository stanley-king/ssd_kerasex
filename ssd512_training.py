...
scales_pascal = [0.07, 0.15, 0.3, 0.45, 0.6, 0.75, 0.9, 1.05]  # The scales for MS COCO are
...
steps = [8, 16, 32, 64, 128, 256, 512]  # The space between two adjacent anchor box center points for each predictor
# layer.
offsets = [0.5, 0.5, 0.5, 0.5, 0.5, 0.5,
           0.5]  # The offsets of the first anchor box center points from the top and left
# borders of the image as a fraction of the step size for each predictor layer.
clip_boxes = False  # Whether or not to clip the anchor boxes to lie entirely within the image boundaries
variances = [0.1, 0.1, 0.2, 0.2]  # The variances by which the encoded target coordinates are divided
...
if not os.path.exists(model_path):
    ...
    weights_path = '/Users/stanley/.keras/models/VGG_ILSVRC_16_layers_fc_reduced.h5'
...
train_path = 'dataset_pascal_voc_07+12_trainval.h5'
if os.path.exists(train_path):
    train_dataset = DataGenerator(load_images_into_memory=False, hdf5_dataset_path=train_path)
else:
    ...
    train_dataset.parse_xml(images_dirs=[VOC_2007_images_dir, VOC_2012_images_dir],
                            ...
                            ret=False)
    train_dataset.create_hdf5_dataset(file_path=train_path,
                                      resize=False,
                                      variable_image_size=True,
                                      verbose=True)

val_path = 'dataset_pascal_voc_07_test.h5'
if os.path.exists(val_path):
    val_dataset = DataGenerator(load_images_into_memory=False, hdf5_dataset_path=val_path)
else:
    ...
    val_dataset.parse_xml(images_dirs=[VOC_2007_images_dir],
                          ...
                          ret=False)
    val_dataset.create_hdf5_dataset(file_path=val_path,
                                    resize=False,
                                    variable_image_size=True,
                                    verbose=True)
...
model_checkpoint = ModelCheckpoint(
    filepath='ssd512_pascal_07_epoch-{epoch:02d}_loss-{loss:.4f}_val_loss-{val_loss:.4f}.h5',
    monitor='val_loss',
    verbose=1,
    save_best_only=True,
...
csv_logger = CSVLogger(filename='ssd512_pascal_07+12_training_log-lr_0.0001.csv',
                       separator=',',
                       append=True)
